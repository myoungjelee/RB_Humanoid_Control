...
- 다음 단계
  - 포트폴리오 정리
  - M10 Observer / Estimator 고도화 진입 준비
...
| M8 | 같은 외란에서 balance feedback OFF/ON 비교 | `fall_event`, `disturb_kpi`, split-screen 비교 | [base scenario](ros2_ws/src/rb_controller/config/scenarios/stand_pd_balance_base.yaml), [run script](ops/run_m8_pair.sh) |
| M9 | M8 결과 자동 요약/비교 | `kpi.json`, `comparison.json`, `summary.md`, `m9/index.csv` | [extractor](scripts/sim2real/extract_m8_kpi.py), [M9 run script](ops/run_m9_kpi.sh), [M8+M9 wrapper](ops/run_m8_and_m9.sh) |
...
### 현재 M8 로그 아티팩트
- disturbance A/B scenario: [stand_pd_balance_base.yaml](ros2_ws/src/rb_controller/config/scenarios/stand_pd_balance_base.yaml)
- 대표 run
...
### 현재 M9 로그 아티팩트
- KPI extractor: [extract_m8_kpi.py](scripts/sim2real/extract_m8_kpi.py)
- runner
  - [run_m9_kpi.sh](ops/run_m9_kpi.sh)
  - [run_m8_and_m9.sh](ops/run_m8_and_m9.sh)
- 대표 아티팩트 위치
...
- 다음 게이트: 포트폴리오 정리 완료 후 M10 Observer / Estimator 고도화

## 다음 단계
- 포트폴리오 정리: README / overview / one-pager / 영상 캡처 최종 반영
