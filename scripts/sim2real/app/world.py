"""Isaac world creation and runtime helpers.

This module keeps the world-facing code in one place:
- task/env resolution
- direct-spawn scene construction
- startup seed/dump helpers
- stepped runtime hooks such as disturbance and fall watch
"""
...
# ---------------------------------------------------------------------------
# spawn path / startup dump helper
# ---------------------------------------------------------------------------
...
class StandaloneDirectWorld:
    ...
    def __init__(
        ...
        # Fall watch는 standing/disturbance phase에서
        # 로봇이 높이 threshold 아래로 확실히 무너지면
        # run을 중단하거나 로그 마커를 남기기 위해 쓴다.
...
def create_standalone_world(args_cli: Any, phase_cfg: dict[str, Any]):
    ...
    # startup root/joint state를 명시적으로 다시 써서
    # 실제 sim 시작 pose와 controller 쪽 seed pose가 어긋나지 않게 맞춘다.
