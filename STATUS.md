...
- 공개용 문서 세트도 다시 동기화한다. `README`는 포트폴리오 랜딩 페이지 구조를 유지한 채 `final demo video`, `system_architecture`, `milestone_roadview` 자산을 직접 연결하고, `overview / ONE_PAGER / ROADMAP / AGENTS / KIST insert / general insert / VIDEO_CAPTURE_GUIDE`도 같은 milestone 체계와 milestone-first 증빙 경로로 맞춘다. `reports/sim2real/video/RB_Humanoid_Control.mp4`를 canonical final demo asset으로 본다.
