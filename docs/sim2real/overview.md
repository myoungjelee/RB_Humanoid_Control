...
Stage1(`docs/stage1/*`)은 baseline 문서로 유지하고, 본 문서는 ROS2 메인 트랙이 **왜 이런 순서로 진행됐는지**, **각 단계에서 무엇을 구현했고 무엇으로 검증했는지**, **현재 어디까지 왔는지**를 기록합니다.

README는 외부 평가자가 먼저 보는 포트폴리오 랜딩 페이지이고, 이 문서는 README를 뒷받침하는 기술 복기 문서 역할을 맡습니다.
...
- 현재 핵심 성과:
  ...
  - 현재 제어 실행 경로를 `State Estimation -> Standing Controller Plugin -> command_raw -> Safety Layer -> command_safe` 구조로 재정리
- 현재 공개 자산:
  - `docs/assets/sim2real/images/standalone_backend/system_architecture.png`
  - `docs/assets/sim2real/images/standalone_backend/milestone_roadview.png`
  - `GitHub attachment final demo`
- 다음 단계:
...
- 검증:
  ...
  - `use_sim_time` 확인
- 증빙:
  - `docs/assets/sim2real/images/standalone_backend/m1_standalone.png`
...
- 증빙:
  - `docs/assets/sim2real/images/standalone_backend/m2_controller_standalone.png`
...
- 증빙:
  - `docs/assets/sim2real/images/standalone_backend/m3_command_standalone.png`
...
- 증빙:
  - `docs/assets/sim2real/images/standalone_backend/m4_clamp_standalone.png`
  - `docs/assets/sim2real/images/standalone_backend/m4_joint_limit_standalone.png`
  - `docs/assets/sim2real/images/standalone_backend/m4_timeout_standalone.png`
  - `docs/assets/sim2real/images/standalone_backend/m4_tilt_standalone.png`
...
- 구조 메모:
  - 당시에는 standing 자체를 먼저 살리기 위해 경량 bring-up controller 경로로 원인 분리를 진행했다
  - 현재 런타임은 이후 리팩을 통해 `State Estimation -> Standing Controller Plugin -> command_raw -> Safety Layer -> command_safe` 구조로 재정리되었다
- 여기서 한 일:
...
## 7) 현재 남은 작업
1. M10 estimator / observer 고도화 + `ros2_control` 기반 제어 경계 정리
2. M11 balance metrics
3. M8 disturbance ON/OFF launch override 정합성 재검토
